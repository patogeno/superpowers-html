...
### Responsive HTML artifacts (2026-08-23, fork version 6.4.1)

The fork's human-facing artifacts are read on whatever device is at hand — desktop, tablet, or phone. They did not survive the trip: on a 375px viewport a committed design spec rendered 413px wide, so prose was clipped at the right edge on every screen, the full-height table of contents consumed the entire first screen before a word of the spec, and diagrams scaled their 12.5px labels down to roughly 5px.

- **Root cause: a CSS-grid blowout.** `.layout` used `grid-template-columns:270px 1fr`, and a `1fr` track with the default `min-width:auto` grows to its widest child — so one wide table, `<pre>`, or diagram stretched the whole page and the sidebar with it. Both tracks are now `minmax(0,…)`, `.main` carries `min-width:0`, and wide content scrolls inside itself instead: bare `<table>`s below 900px, `pre` at any width, plus an optional `.table-wrap`.
- **A real phone layout.** Below 900px (tablet portrait and every phone) the two-column shell collapses, the sidebar becomes a compact sticky bar, and the table of contents becomes a horizontally scrollable row of chips. Breakpoints at 1100 / 900 / 640 / 560px, a landscape-phone guard that un-sticks the bar on short screens, and `--nav-h`-aware `scroll-margin-top` so a chip jump does not park the heading under the bar.
- **Diagrams stay legible.** `.fig svg` scales to the column. A diagram whose `viewBox` is wider than 640 user units must now be marked `<div class="fig wide">`: it keeps a legible floor width and scrolls horizontally inside its figure, with a caption hint, rather than shrinking to unreadable. The three committed example specs were re-inlined with the new stylesheet and their wide figures marked; the 2026-05-30 spec also moved onto the canonical `.layout` shell.
- **The rule binds mockups too.** `html-artifacts` gains rule 6 (responsiveness), alongside self-containment, as the second rule a product-styled mockup reuses — it carries its own theme, so it carries its own fluid widths and breakpoint. `references/authoring.md` gains a Responsiveness section and phone-aware diagram rules; the example mockup now models them.
- **Enforced, not just documented.** New `findResponsiveDeficiencies()` in `tests/html-artifacts/validate.js` flags a missing or non-`device-width` viewport meta, a hard-coded CSS container width at or above 480px, an `<svg>` without a `viewBox` or with an absolute size attribute, and a diagram over 640 units missing `.fig.wide`. 12 new tests; the `test:html` suite is 59 green. Verified in a real browser across four artifacts × six viewport widths (320–1280px): zero horizontal page overflow.
...
- **Merged upstream v6.2.0.** Brings the subagent-driven-development lifecycle rewrite (plan-scoped `.superpowers/sdd/<plan>/` workspaces, resume-based fix rounds with a five-round circuit breaker, one reviewer per task, file-based task/diff handoffs), the library-wide skill compression, `writing-good-tests.md`, Kimi/Pi/Antigravity harness support, and the Windows SessionStart `shell: "bash"` fix. The fork's `html-artifacts` wiring in `brainstorming` merged cleanly; `writing-plans` kept both upstream's Global Constraints block and the fork's `**Execution:**` line, with the Specialist tag moved after upstream's new `**Interfaces:**` block.
- **New `claude-5-models.md` in `subagent-driven-development`.** Maps the skill's deliberately vendor-neutral tiers — cheap / standard / most capable — onto Claude Haiku 4.5, Sonnet 5, Opus 5, and Fable 5 with current prices and context limits, and records the dispatch-prompt shifts that matter on Claude 5: don't add verification instructions (these models self-verify, and telling them to double-check produces over-verification), don't add "delegate more" guidance (Opus 5 already reaches for subagents readily), state scope as a boundary, and never cap a reviewer to high-severity findings (Claude 5 follows that filter literally, so recall falls while review quality rises). Fable 5 is documented as an escalation past Opus 5, not the default top tier — it costs 2×, refuses more, and is unavailable under zero data retention. **Additive only:** upstream's neutral vocabulary is untouched and asserted by a test, so non-Anthropic harnesses are unaffected.
- **Progressive disclosure in the fork's `writing-plans` additions.** The team-plan mechanics and the multi-session folder layout moved into sibling `team-plans.md` and `multi-session-plans.md`; `SKILL.md` keeps the decisions (which execution model, whether the work spans sessions) and points at the detail. Both paths are minority paths, so a typical planning session no longer carries them. A new test asserts every sibling file `SKILL.md` points at actually exists.
- **Reconciled with the mockups work (6.1.0).** This release also merges the mockups-replace-visual-companion branch, which had landed independently on top of upstream v6.0.0. Both feature sets survive: `brainstorming` keeps its *Mockups* section (upstream's *Visual Companion* heading is gone, as intended) and `writing-plans` keeps the header's `**Mockups:**` line and its *Mockups* section alongside the `**Execution:**` line and upstream's *Global Constraints* block. Upstream's `Key Principles` section stays deleted, matching the v6.1.0/v6.2.0 compression campaign. `.kimi-plugin/plugin.json` — added by the upstream merge and missed in the first bump — is now on 6.3.0 with the other five manifests.
